"""Loading, feature engineering and plotting for the robot execution dataset.

Authors: Ilyess Assadi, Adel Bousri.

Provides:
- a parser for the UCI Robot Execution Failures format, where a label line is
  followed by 15 lines of 6 sensor readings
- binary and multiclass label encoding
- statistical features extracted from each execution's time series
- plotting helpers and evaluation metrics used by the notebooks

The single most important thing in this file is `HEALTHY_LABELS`. LP3 calls its
healthy class 'ok' and never uses 'normal'; matching on 'normal' alone labelled
20 healthy executions as failures and made that subset look entirely faulty.
"""
...
#: Les étiquettes qui désignent une exécution saine, tous sous-ensembles confondus.
#: 'ok' est celle de LP3, qui n'emploie jamais 'normal'. Toute étiquette absente
#: de cet ensemble est une défaillance.
HEALTHY_LABELS = frozenset({"normal", "ok"})

#: Colonnes de service, ni features ni capteurs. Elles accompagnent les données
#: sans jamais entrer dans un modèle : `label_encoded` et `label_binary` sont la
#: cible, et les inclure donnerait un score parfait pour de mauvaises raisons.
        ...
        try:
            ...
            while i < len(lines):
                ...
                if data_lines_read == samples_per_instance:
                    ...
                    instance = {"label": label, "source": source}
...
def encode_labels(df: pd.DataFrame, binary: bool = False) -> tuple[pd.DataFrame, dict]:
    ...
    if binary:
        # Binary encoding: healthy=0, every failure label=1.
        #
        # LP3 names its healthy class 'ok', not 'normal'. It is the only subset
        # that does: LP1, LP2, LP4 and LP5 all use 'normal', and LP3 contains no
        # 'normal' row at all. Matching on 'normal' alone therefore labelled the
        # 20 healthy LP3 executions as failures, and made an entire subset look
        # 100% anomalous.
        #
        # Counting from the raw files: 109 'normal' + 20 'ok' = 129 healthy runs
        # out of 463, so 72% anomalies rather than the 76% that the mistake
        # produced. Every score computed on the previous encoding is affected,
        # starting with the majority-class baseline.
        df_encoded["label_binary"] = df_encoded["label"].apply(
            lambda x: 0 if str(x).lower() in HEALTHY_LABELS else 1
        )
...
def create_statistical_features(
    df: pd.DataFrame, samples_per_sensor: int = 15, num_sensors: int = 6
) -> pd.DataFrame:
    """
    ...
    Returns:
        pd.DataFrame: DataFrame with the 48 statistical features, plus the
            label and source columns.
    ...
    if len(feature_cols) != samples_per_sensor * num_sensors:
        print("⚠️  Format not recognized. Using all numeric features.")
        feature_cols = [col for col in df.columns if df[col].dtype in [np.int64, np.float64]]
        feature_cols = [
            col
            for col in feature_cols
    ...
    n_stats = len(stats_df.columns) - len(META_COLUMNS & set(stats_df.columns))
    print(f"✅ Statistical features created: {len(stats_df.columns)} columns")
    print(f"   Original features: {len(feature_cols)}")
    print(f"   Statistical features: {n_stats}")
