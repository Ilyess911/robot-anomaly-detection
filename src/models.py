"""
Machine Learning models module for supervised and unsupervised anomaly detection.
Author: Thomas Martin

This module provides classes and functions for:
- Supervised learning models (Logistic Regression, Random Forest, SVM, Gradient Boosting)
- Unsupervised anomaly detection (Isolation Forest, One-Class SVM)
- Model training with GridSearchCV and cross-validation
...
class SupervisedModels:
    ...
    def __init__(self):
        """Initialize SupervisedModels with empty dictionaries for models, parameters, and results."""
        self.models = {}
        self.best_params = {}
        self.scalers = {}
        self.results = {}
    ...
    def train_logistic_regression(self,
                                  ...
                                  cv: int = 5,
                                  n_jobs: int = -1,
                                  scoring: str = 'f1_weighted') -> LogisticRegression:
        """
    ...
    def train_random_forest(self,
                           ...
                           cv: int = 5,
                           n_jobs: int = -1,
                           scoring: str = 'f1_weighted') -> RandomForestClassifier:
        """
    ...
    def train_gradient_boosting(self,
                               ...
                               cv: int = 5,
                               n_jobs: int = -1,
                               scoring: str = 'f1_weighted') -> GradientBoostingClassifier:
        """
        ...
        grid_search = GridSearchCV(gb, param_grid, cv=cv, scoring=scoring,
                                   n_jobs=n_jobs, verbose=1, return_train_score=True)
        grid_search.fit(X_train, y_train)
    ...
    def predict(self, model_name: str, X: np.ndarray, return_proba: bool = False) -> np.ndarray:
        ...
        Args:
            model_name (str): Name of the model ('logistic_regression', 'random_forest', 'svm', 'gradient_boosting').
        ...
        Returns:
            np.ndarray: Predicted labels. If return_proba=True, returns tuple (predictions, probabilities).
        ...
        if return_proba:
            if hasattr(model, 'predict_proba'):
                return model.predict(X), model.predict_proba(X)[:, 1]  # Return probability of positive class
            else:
...
def train_supervised_model(model_type: str,
                          X_train: np.ndarray,
                          y_train: np.ndarray,
                          **kwargs) -> object:
    """
    ...
    Args:
        model_type (str): Type of model ('logistic_regression', 'random_forest', 'svm', 'gradient_boosting').
        X_train (np.ndarray): Training features (n_samples, n_features).
        y_train (np.ndarray): Training labels (n_samples,).
...
def train_one_class_svm(X_train: np.ndarray,
                        ...
                        kernel: str = 'rbf',
                        gamma: str = 'scale',
                        random_state: int = 42) -> Tuple[OneClassSVM, StandardScaler]:
    """
    ...
    Returns:
        Tuple[OneClassSVM, StandardScaler]: Trained model and scaler (scaler needed for predictions).
...
def compare_models(results_dict: Dict[str, Dict],
    ...
    Args:
        ...
        metric (str): Metric to compare (e.g., 'accuracy', 'f1_score', 'auc'). Default is 'accuracy'.
    ...
    plt.bar(model_names, metric_values, 
           color=colors[:len(model_names)], edgecolor='black', alpha=0.7)
    ...
    if metric_values:
        ...
        for i, v in enumerate(metric_values):
            plt.text(i, v + max_val * 0.01, f'{v:.3f}', 
